...
export type Transition<TPhase extends Phase = Phase> =
  | TransitionToNext
...
type Test = UnpackExecutes<
  [
    Execute<{ type: 'subphase'; subphase: TargetPhase }>,
    Execute<TransitionToSelf, [TargetPhase]>
  ]
>

type IsTrue = [
  Execute<TransitionToSubPhase<TargetPhase>>,
  Execute<TransitionToSelf, [TargetPhase]>
] extends [
  infer Head extends Execute<TransitionToSubPhase<infer TPhase>, []>,
  infer Rest
]
  ? [Head, TPhase]
  : false
...
type IsTrue2 = [
  Execute<TransitionToSubPhase<TargetPhase>>,
  Execute<TransitionToSelf, [TargetPhase]>
] extends [infer Head extends Execute, infer Rest]
  ? [Head, TransitionResult<ReturnType<Head>>]
  : false

function executeSequence<T>(...executes: UnpackExecutes<T>): UnpackExecutes<T> {
  return executes
}
